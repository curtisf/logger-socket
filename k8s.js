...
module.exports = {
  ...
  renegotiate: async function (force = false) {
    ...
    if (client.shardId !== 0 && !force) return logger.log('K8S-SCALE', 'Renegotiation cancelled, this client does not manage shard 0')
    ...
    if (discord.shards > workload.body.spec.replicas) {
      logger.log('K8S-SCALE', `Scale inadequate! Discord recommends ${discord.shards} shards but client has ${client.shardCount}`)
  ...
  determine: async function (autoconnect = true) {
    ...
    if (client.shardCount !== thisworkload.length) {
      logger.log('K8S-SCALE', 'This pod is not supporting required shards, reidentifying...')
      client.autoReconnect = false
      client.disconnect()
      client.shardId = index
      client.shardCount = thisworkload.length
      client.presence = {
        activity: {
          name: `shard ${index}`,
          type: 0
